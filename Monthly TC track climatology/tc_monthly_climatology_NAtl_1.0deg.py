...
# North Atlantic tropical cyclone tracks using IBTrACS data
...
all_TC_tracks = pd.read_csv('C:/Users/Brian/Desktop/CSci135 (ComSci in Meteorology)/ibtracs.NA.list.v04r00.csv',
...
def plot_monthly_tracks(month):
    ...
    hist = np.zeros((70, 110)) # 0-70N, 0-110W
    ...
    for tc in monthly_TCs:
        for sid, locations in tc.items():
            ...
            for lat, lon in locations:
                ...
                lat_index = int(lat - 0)
                lon_index = int(lon - (-110))
                # Check if the indices are within the expected range
                if 0 <= lat_index < 70 and 0 <= lon_index < 110:
    ...
    xedges = np.arange(-110, 1, 1)  # 110 elements
    yedges = np.arange(0, 71, 1)  # 70 elements

    fig = plt.figure(figsize=(25., 25.), dpi=250)
    ax = plt.axes(projection=ccrs.PlateCarree())
    ax.set_extent([-110, 0, 0, 70], ccrs.PlateCarree())

    gridlines = ax.gridlines(draw_labels=False, xlocs=np.arange(-180, 181, 5), ylocs=np.arange(-90, 91, 5), color='gray', linestyle='--')
    xticks = np.arange(-110, 1, 5)
    yticks = np.arange(0, 71, 5)
    ...
    fig.suptitle('IBTrACS | Monthly climatology of North Atlantic tropical cyclone tracks (1851–2023)', fontsize=30, y=0.793)
