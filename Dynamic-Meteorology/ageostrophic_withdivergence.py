...
# Ver. 2 (divergence in shaded contours in the background)
...
for time in times:
    ...
    divergence = mpcalc.divergence(uwind_data, vwind_data)
    divergence_visuals = ax.contourf(x, y, divergence, transform=ccrs.PlateCarree(), levels=np.arange(-0.00008, 0.00008, 0.0000001), cmap='BrBG')
    ...
    norm = Normalize(vmin=-0.00008, vmax=0.00008)
    colorbar = ColorbarBase(cax, cmap='BrBG', norm=norm, extend='both', ticks=np.arange(-0.00008, 0.00009, 0.000002))
    colorbar.set_label('Divergence (1/s)')
    
    # Adding plot title and captions
    title = 'Wind flow at the ' + str(press_level) + ' hPa level, ' + time_current[:10] + ' ' + time_current[11:16] + ' UTC'
    caption = 'Geopotential height (dam, black solid isohypses), 6-hour geopotential height change (dam, red/blue dashed-dotted isallohypses), divergence (1/s, shading),\nactual wind velocity (m/s, lime-green arrow), geostrophic wind (m/s, red arrow), ageostrophic wind (m/s, blue arrow)'
