...
# North Indian tropical cyclone tracks using IBTrACS data
...
all_TC_tracks = pd.read_csv('C:/Users/Brian/Desktop/CSci135 (ComSci in Meteorology)/ibtracs.NI.list.v04r00.csv',
...
def plot_monthly_tracks(month):
    ...
    hist = np.zeros((45, 65)) #5S-40N, 40-105E
    ...
    for tc in monthly_TCs:
        for sid, locations in tc.items():
            ...
            for lat, lon in locations:
                ...
                lat_index = int(lat - (-5))
                lon_index = int(lon - 40)
                # Check if the indices are within the expected range
                if 0 <= lat_index < 45 and 0 <= lon_index < 65:
    ...
    fig.suptitle('IBTrACS | Monthly climatology of North Indian Ocean tropical cyclone tracks (1842–2023)', fontsize=30, y=0.814)
