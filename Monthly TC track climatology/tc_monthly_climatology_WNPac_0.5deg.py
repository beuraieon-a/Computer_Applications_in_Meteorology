...
# Western North Pacific tropical cyclone tracks using IBTrACS data
...
def plot_monthly_tracks(month):
    ...
    for tc in monthly_TCs:
        for sid, locations in tc.items():
            ...
            for lat, lon in locations:
                ...
                lon_index = int((lon - 100) * 2)
                # Check if the indices are within the expected range
                if 0 <= lat_index < 120 and 0 <= lon_index < 160:
    ...
    lons_PAR = [120, 135, 135, 115, 115, 120, 120]
    lats_PAR = [25, 25, 5, 5, 15, 21, 25]
    ax.plot(lons_PAR, lats_PAR, color='black', linewidth=3, transform=ccrs.PlateCarree(), linestyle='-')
    
    lons_TCAD = [114, 114, 145, 145, 114]
    lats_TCAD = [4, 27, 27, 4, 4]
    ax.plot(lons_TCAD, lats_TCAD, color='black', linewidth=3, transform=ccrs.PlateCarree(), linestyle='--')
    
    lons_TCID = [110, 110, 155, 155, 110]
    lats_TCID = [0, 27, 27, 0, 0]
    ax.plot(lons_TCID, lats_TCID, color='black', linewidth=3, transform=ccrs.PlateCarree(), linestyle=':')
    ...
    fig.suptitle('IBTrACS | Monthly climatology of Western North Pacific tropical cyclone tracks (1884–2023)', fontsize=25, y=0.835)
