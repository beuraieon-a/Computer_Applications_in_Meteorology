...
# Central and Eastern North Pacific tropical cyclone tracks using IBTrACS data
...
all_TC_tracks = pd.read_csv('C:/Users/Brian/Desktop/CSci135 (ComSci in Meteorology)/ibtracs.EP.list.v04r00.csv',
...
def plot_monthly_tracks(month):
    ...
    hist = np.zeros((65, 105)) # 0-65N, 75-180W
    ...
    for tc in monthly_TCs:
        for sid, locations in tc.items():
            ...
            for lat, lon in locations:
                ...
                lat_index = int(lat - 0)
                lon_index = int(lon - (-180))
                # Check if the indices are within the expected range
                if 0 <= lat_index < 65 and 0 <= lon_index < 105:
    ...
    fig.suptitle('IBTrACS | Monthly climatology of Central and Eastern North Pacific tropical cyclone tracks (1876–2023)', fontsize=26, y=0.785)
